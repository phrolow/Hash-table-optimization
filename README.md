...
В этом проекте я исследовал оптимизацию времени поиска в хеш-таблицах. Моя работа не ставит целью оптимизировать заполнение хеш-таблицы, удаление элементов из неё или же расход памяти. Я оставляю за собой право оптимизировать поиск в ущерб этим факторам, хотя по возможности буду их учитывать.
## О хеш-таблицах

**Хеш-таблица** — это структура данных, которая позволяет хранить пары (ключ, значение) и выполнять три операции: операцию добавления новой пары, операцию удаления и операцию поиска пары по ключу.
...
- Разброс длин списков. 

Разброс длин списков - очень важный фактор для хеш-функции. Чем равномернее распределены длины списков, тем быстрее будет поиск для слов, оказавшихся в списке подлиннее.

В моей работе двум одинаковым словам соответствует одно место в списке. Недостатки программы, для которой каждому слову соответствует своё значение, следующие:
 - На распределение длин списков влияет частота встречаемости слов. Следовательно, сложнее оценить фактор хеш-функции.
 - Неприменимость на практике. Для каждого слова из набора одинаковых слов функция поиска будет возвращать одно и то же значение; следовательно, остальные такие же элементы списка не используются.
...
Тем не менее, множество значений функции сильно ограничено. Для большого числа данных функция неэффективна. 

Иллюстрация для большого, простого размера таблицы **4973**:

![hashSum](/images/histogram_hashsum_big.png)
 ### hashRol
